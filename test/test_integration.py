...
class MockLLMService:
    ...
    def summarize(self, transcript: str) -> Summary:
        from entities.metting import Task, Decision
        return Summary(
...
class TestEndToEndWorkflow:
    ...
    def test_complete_solo_workflow(self):
        ...
        s_result = summarize_uc.execute(SummarizeMeetingInput(
            meeting=meeting
        ))
    ...
    def test_multiple_participants(self):
        ...
        assert abs(meeting.duration_minutes - 0.4166666666666667) < 0.0001  # Tolerância para float
...
class TestDataConsistency:
    ...
    def test_summary_tasks_format(self):
        ...
        from entities.metting import Task
        ...
        # Cada tarefa deve ter os campos obrigatórios
    ...
    def test_meeting_transcript_consistency(self):
        ...
        # Full text deve estar contido no formatted
...
class TestErrorRecovery:
    ...
    def test_partial_processing_recovery(self):
        ...
        transcriber = WhisperTranscriber()  # Transcriber real, não mock
        uc = TranscribeMeetingUC(transcriber)
        
        # Tenta com arquivo inválido - vai retornar erro_message
        result = uc.execute(TranscribeMeetingInput(
            audio_path="/tmp/nonexistent_file_12345_xyz.wav"
        ))
        
        # Deve retornar erro ou False, não lançar exception
        # O sistema continua funcionando mesmo após erro
        assert isinstance(result.success, bool)  # Não lançou exception
        if not result.success:
            assert result.error_message  # Tem mensagem de erro
    ...
    def test_corrupted_json_handling(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Tenta carregar (deve falhar gracefully)
            try:
                loaded = repo.find_by_id("test")
                # Se chegou aqui, a corrupção foi tratada
